...
const s = {
  ...
  modal:      { background: 'var(--c-bg)', borderRadius: 16, padding: '1.5rem', width: 400, boxShadow: '0 20px 60px rgba(0,0,0,0.25)', display: 'flex', flexDirection: 'column', gap: 0 },
  modalHead:  { display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 },
  modalTitle: { margin: 0, fontSize: 16, fontWeight: 800, color: 'var(--c-text)' },
  closeBtn:   { background: 'none', border: 'none', color: 'var(--c-muted)', cursor: 'pointer', fontSize: 18, padding: 4 },
  label:      { display: 'block', fontSize: 11, fontWeight: 700, color: 'var(--c-muted)', marginBottom: 4, textTransform: 'uppercase', letterSpacing: .4 },
  input:      { width: '100%', padding: '9px 11px', border: '1.5px solid var(--c-border-mid)', borderRadius: 8, fontSize: 13, color: 'var(--c-text)', outline: 'none', boxSizing: 'border-box' },
  checkLabel: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: 'var(--c-text)', cursor: 'pointer' },
  saveBtn:    { padding: '9px 20px', background: 'var(--c-primary)', color: '#fff', border: 'none', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: 'pointer' },
  cancelBtn:  { padding: '9px 16px', background: 'var(--c-bg)', border: '1.5px solid var(--c-border-mid)', borderRadius: 8, cursor: 'pointer', color: 'var(--c-muted)', fontSize: 13 },

  emptyBox:   { textAlign: 'center', padding: '4rem', background: 'var(--c-bg)', borderRadius: 14, border: '1px solid var(--c-border)' },
  emptyTitle: { margin: '0 0 8px', fontSize: 16, fontWeight: 700, color: 'var(--c-text)' },
  emptySub:   { margin: 0, fontSize: 13, color: 'var(--c-muted)' },

  grid:       { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 14 },
  card:       { background: 'var(--c-bg)', border: '1px solid var(--c-border)', borderRadius: 14, padding: '1.1rem 1.25rem', boxShadow: 'var(--c-shadow-sm)', display: 'flex', flexDirection: 'column', gap: 14 },
  cardHead:   { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 10 },
  cardNombre: { margin: '0 0 3px', fontSize: 15, fontWeight: 700, color: 'var(--c-text)' },
  cardDir:    { margin: 0, fontSize: 12, color: 'var(--c-muted)' },
  ...
  editBtn:    { flex: 1, padding: '7px', background: 'var(--c-primary-light)', color: 'var(--c-primary)', border: 'none', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer' },
